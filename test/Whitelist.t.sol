...
contract WhitelistHookTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(address(this), flags, type(WhitelistHook).creationCode, abi.encode(address(manager)));
        whitelist = new WhitelistHook{salt: salt}(IPoolManager(address(manager)));
        require(address(whitelist) == hookAddress, "WhitelistHookTest: hook address mismatch");

        // Create the pool
        key = PoolKey(currency0, currency1, 3000, 60, IHooks(address(whitelist)));
        poolId = key.toId();
    ...
    function testAddToWhitelist() public {
        ...
        assertEq(whitelist.whitelisted(user1), true);
